...
import hlm.excel.util.v2.extractor.ColumnMetadataExtractor;
import hlm.excel.util.v2.extractor.FieldExtractor;
import hlm.excel.util.v2.util.CellUtils;
...
/**
 * The ExcelMapper class is responsible for converting a list of POJOs into an Excel workbook representation.
...
public class ExcelMapper {
     ...
     * The list should contain objects of a class annotated with @ExcelSheet to map the sheet name and columns.
     *
     * @param dataList the list of POJOs to map to the Excel workbook.
     * @return a Workbook object containing the data.
     * @throws IllegalArgumentException if the data list is empty or the POJO class is not annotated with @ExcelSheet.
     */
    public Workbook mapToExcel(List<?> dataList) {
        ...
        // Check if the class has the @ExcelSheet annotation
        ExcelSheet sheetAnnotation = clazz.getAnnotation(ExcelSheet.class);
        if (sheetAnnotation == null) {
            throw new IllegalArgumentException("POJO class must have an @ExcelSheet annotation.");
        }
