package hlm.excel.util.v2.extractor;
...
import hlm.excel.util.v2.util.ClassTypeUtils;
import hlm.excel.util.v2.util.HeaderNameProcessUtils;
import hlm.excel.util.v2.util.ValidationUtils;
 ...
 * <p>
 * This class relies on annotations (`@ExcelSheet`, `@ExcelColumn`) to extract necessary metadata. Additionally, it utilizes a utility
...
public class ColumnMetadataExtractor {
     ...
     * @throws IllegalArgumentException if the class has circular references or is missing the @ExcelSheet annotation.
     */
     ...
     * @throws IllegalArgumentException if the class is missing the @ExcelSheet annotation or contains circular references.
     */
    protected void extractColumnMetadata(Class<?> clazz, String parentHeader, List<ColumnMetadata> metadataList, ArrayList<Field> parentClassList) {
        ...
        // Ensure the class has the required @ExcelSheet annotation.
        ExcelSheet sheetAnnotation = clazz.getAnnotation(ExcelSheet.class);
        if (sheetAnnotation == null) {
            throw new IllegalArgumentException("POJO class must have an @ExcelSheet annotation." + clazz.getName());
        }
