...
class ColumnMetadataExtractorTest {
    ...
    // Test Case 1: Test process with a valid class that has @ExcelSheet and @ExcelColumn annotations
    @Test
    ...
    // Test Case 2: Test process with missing @ExcelSheet annotation
    @Test
    void testProcess_MissingExcelSheetAnnotation() {
        // Act & Assert: Process a class without @ExcelSheet annotation and expect an exception
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            columnMetadataExtractor.process(MyTestObjectWithoutExcelSheet.class);
        });
        assertEquals("POJO class must have an @ExcelSheet annotation." + MyTestObjectWithoutExcelSheet.class.getName(), exception.getMessage());
    }
